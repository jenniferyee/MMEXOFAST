...
class PlanetFitInfo:
        ...
        if self._sfit_params is None:
            for line in self.lines:
                ...
                if "Initial SFit" in line:
                    ...
                    params.pop(
                        "chi2", None
                    )  # chi2 is now included but isn't a model param
    ...
    def planet_t_range(self):
        if self.mag_methods is not None:
            ...
            start = (
                ...
                - n_teff * self.best_af_grid_point["t_eff"]
            )
            stop = (
                self.best_af_grid_point["t_0"]
                + n_teff * self.best_af_grid_point["t_eff"]
    ...
    def plot_planet_window(self):
        if self.best_af_grid_point is not None:
            plt.axvline(
                self.best_af_grid_point["t_0"] - 2450000.0,
                color="black",
                linestyle=":",
            )
            plt.axvline(
                self.best_af_grid_point["t_0"]
                - self.best_af_grid_point["t_eff"]
                - 2450000.0,
                color="black",
                linestyle="--",
            )
    ...
    def make_plot(self, event, n_tE=5):
        ...
        plt.xlim(np.array(self.get_event_t_range(n_tE=n_tE)) - 2450000.0)
        plt.minorticks_on()

        plt.subplot(1, 2, 2)
        event.plot_data(show_bad=True, subtract_2450000=True)
        event.plot_model(
            t_range=self.planet_t_range,
            color="black",
            subtract_2450000=True,
            zorder=10,
        )
        self.plot_planet_window()
        plt.xlim(np.array(self.planet_t_range) - 2450000.0)
        plt.minorticks_on()
    ...
    def plot_revised_pspl_fit(self):
        ...
        event = MulensModel.Event(
            datasets=self.fitter.masked_datasets, model=model
        )
        self.make_plot(event)
