...
class AllResults:
    ...
    def __init__(self, path="."):
        ...
        self.colors = {
            AllResults.fit_types[i]: AllResults._colors[i]
            for i in range(len(AllResults.fit_types))
        }
    ...
    def get_results(self, path):
        ...
        for file in np.sort(logs):
            ...
            for fit_type, params in zip(
                ...
                [
                    planet.initial_pspl_guess,
                    planet.sfit_params,
                    planet.revised_sfit_params,
                ...
                if results[fit_type] is None:
                    ...
                    results[fit_type] = pd.concat(
                        [results[fit_type], df], axis=1
                    )
    ...
    def get_answers(self):
        ...
        for value in self.results["Initial PSPL Guess"]["ID"].values:
            key = int(value)
            df = pd.concat(
                (pd.Series(data={"ID": key}), all_answers.data.iloc[key - 1])
            )
    ...
    def _make_hist(self, key, frac=False, **kwargs):
        ...
        for fit_type in fit_types:
            if frac:
                x = (
                    self.__getattribute__("delta_{0}".format(key))[fit_type]
                    / self.answers[self.get_ans_key(key)]
                )
            ...
            plt.hist(
                ...
                label="{0} ({1})".format(
                    ...
                    np.sum(
                        pd.notna(
                            self.__getattribute__("delta_{0}".format(key))[
                                fit_type
                            ]
                        )
                    ),
                ),
                edgecolor=self.colors[fit_type],
                lw=2,
                facecolor="none",
                **kwargs,
            )
        ...
        for fit_type in AllResults.fit_types:
            ...
            for key in self.results[fit_type].columns:
                if key != "ID":
                    ...
                    x = self.__getattribute__("delta_{0}".format(key))[
                        fit_type
                    ]
                    ...
                    goodstr += "{0:9.4f} ".format(
                        np.sum(
                            (np.abs(frac_err) < 0.2) & (np.isfinite(frac_err))
                        )
    ...
    def delta_u_0(self):
        if self._delta_u_0 is None:
            ...
            for fit_type in AllResults.fit_types:
                delta_u_0[fit_type] = (
                    self.answers["u0"] - self.results[fit_type]["u_0"]
                )
    ...
    def delta_t_E(self):
        if self._delta_t_E is None:
            ...
            for fit_type in AllResults.fit_types:
                delta_t_E[fit_type] = (
                    self.answers["tE"] - self.results[fit_type]["t_E"]
                )
    ...
    def delta_rho(self):
        if self._delta_rho is None:
            ...
            for fit_type in AllResults.planet_fit_types:
                delta_rho[fit_type] = (
                    self.answers["rhos"] - self.results[fit_type]["rho"]
                )
    ...
    def delta_s(self):
        if self._delta_s is None:
            ...
            for fit_type in AllResults.planet_fit_types:
                delta_s[fit_type] = (
                    self.answers["s"] - self.results[fit_type]["s"]
                )
    ...
    def delta_q(self):
        if self._delta_q is None:
            ...
            for fit_type in AllResults.planet_fit_types:
                delta_q[fit_type] = (
                    self.answers["q"] - self.results[fit_type]["q"]
                )
    ...
    def delta_alpha(self):
        if self._delta_alpha is None:
            delta_alpha = {}
            for fit_type in AllResults.planet_fit_types:
                delta_alpha[fit_type] = (
                    self.answers["alpha"] - self.results[fit_type]["alpha"]
                )
