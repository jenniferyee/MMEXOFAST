...
class TestDatasetInitialization(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Define file paths
        cls.ob140939_ogle = str(
            cls.ob140939_dir / "n20100310.I.OGLE.OB140939.txt"
        )
        cls.ob140939_spitzer = str(
            cls.ob140939_dir / "n20140605.L.Spitzer.OB140939.txt"
        )

        cls.ob05390_ogle = str(
            cls.ob05390_dir / "n20010804.I.OGLE.OB05390.txt"
        )
    ...
    def test_1_datasets_only_single_ground(self):
        ...
        dataset_ogle = MulensModel.MulensData(
            file_name=self.ob140939_ogle, phot_fmt="flux"
        )
    ...
    def test_4_pickle_only_with_renorm_single_ground(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                fitter2.renorm_factors[self.ob140939_ogle_label], 1.5
            )
    ...
    def test_5a_pickle_plus_new_only_same_location_no_renorm(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            dataset_moa = MulensModel.MulensData(
                file_name=self.ob05390_moa, phot_fmt="flux"
            )
            restart_file = os.path.join(tmpdir, f"{file_head}_restart.pkl")
    ...
    def test_6a_pickle_plus_new_only_same_location_with_renorm(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(len(fitter2.renorm_factors), 1)
            self.assertEqual(
                fitter2.renorm_factors[self.ob05390_ogle_label], 1.3
            )
    ...
    def test_8a_pickle_plus_new_only_different_location_with_renorm(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(len(fitter2.renorm_factors), 1)
            self.assertEqual(
                fitter2.renorm_factors[self.ob140939_ogle_label], 1.2
            )
            self.assertNotIn(
                self.ob140939_spitzer_label, fitter2.renorm_factors
            )
