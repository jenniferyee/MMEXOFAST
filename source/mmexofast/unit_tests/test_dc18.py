...
def _make_upstream_tarball(path, events=(1, 4), bands=('W149', 'Z087'),
                           inside_lc_dir=True):
    """
...
class TestUpstreamNameTranslation(unittest.TestCase):
    ...
    def test_ignores_files_that_are_not_light_curves(self):
        for name in ('event_info.txt', 'README.md',
                     'wfirst_ephemeris_W149.txt', 'master_file.txt'):
...
class TestExtractAndRename(unittest.TestCase):
    ...
    def test_finds_light_curves_outside_the_lc_directory(self):
        ...
        _make_upstream_tarball(
            self.tarball, events=(4,), inside_lc_dir=False)
        written = dc18._extract_and_rename(self.tarball, self.destination)
    ...
    def test_content_is_preserved(self):
        ...
        written = os.path.join(
            self.destination, 'n20180816.W149.DC18.004.txt')
...
class TestFetchLightCurves(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = mock.patch.object(dc18, '_cache_dir',
    ...
    def test_fetches_unpacks_and_caches(self):
        with mock.patch('astropy.utils.data.download_file',
                        side_effect=self._download) as download:
