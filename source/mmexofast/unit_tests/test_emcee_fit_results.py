...
from mmexofast.results import FitRecord, MMEXOFASTFitResults
...
_MOCK_MAG_I_SOURCE_OGLE = 22 - 2.5 * np.log10(
    float(np.squeeze(_SOURCE_FLUXES[0]))
)
_MOCK_MAG_I_BLEND_OGLE = 22 - 2.5 * np.log10(
    float(np.squeeze(_BLEND_FLUXES[0]))
)
_MOCK_MAG_R_SOURCE_MOA = 22 - 2.5 * np.log10(
    float(np.squeeze(_SOURCE_FLUXES[1]))
)
_MOCK_MAG_R_BLEND_MOA = 22 - 2.5 * np.log10(
    float(np.squeeze(_BLEND_FLUXES[1]))
)
...
def make_mock_emcee_fitter(
    parameters_to_fit=None, fixed_params_dict=None, seed=42
):
    """
    ...
    chi2_best = _N_DATA + np.sum(
        ((best_theta - true_values) / step_sizes) ** 2
    )
...
@pytest.fixture
def emcee_fit_results(
    mock_sampler, best_theta, emcee_settings, parameters_to_fit
):
...
class TestEmceeFitResults:
    ...
    def test_stores_parameters_to_fit(
        self, emcee_fit_results, parameters_to_fit
    ):
    ...
    def test_percentiles_use_post_burnin_chain_only(
        ...
        np.testing.assert_array_almost_equal(
            emcee_fit_results.percentiles, expected
        )
    ...
    def test_sigma_minus_is_p50_minus_p16(self, emcee_fit_results):
        ...
        np.testing.assert_array_almost_equal(
            emcee_fit_results.sigma_minus, p[1] - p[0]
        )

    def test_sigma_plus_is_p84_minus_p50(self, emcee_fit_results):
        p = emcee_fit_results.percentiles
        np.testing.assert_array_almost_equal(
            emcee_fit_results.sigma_plus, p[2] - p[1]
        )

    def test_sigmas_is_mean_of_asymmetric(self, emcee_fit_results):
        expected = (
            emcee_fit_results.sigma_minus + emcee_fit_results.sigma_plus
        ) / 2
        np.testing.assert_array_almost_equal(
            emcee_fit_results.sigmas, expected
        )
...
class TestEmceeLCFitterResultsAfterRun:
    ...
    def fitter_after_run(self, fitter_before_run):
        ...
        with (
            patch.object(
                fitter, "initialize_event", side_effect=fake_initialize
            ),
...
class TestMMEXOFASTFitResultsWithEmcee:
    ...
    def test_get_params_uses_linear_space_keys(
    ...
    def test_get_sigmas_keys_match_parameters_to_fit(
        ...
        assert (
            list(full_result.get_sigmas_from_results().keys())
            == parameters_to_fit
        )
    ...
    def test_get_sigmas_reads_from_results_sigmas(
        ...
        expected = dict(
            zip(parameters_to_fit, mock_emcee_fitter.results.sigmas)
        )
    ...
    def test_get_sigmas_are_positive(self, full_result):
        assert all(
            v > 0 for v in full_result.get_sigmas_from_results().values()
        )
    ...
    def test_format_fitted_sigma_minus_are_positive(
        self, df, parameters_to_fit
    ):
        for param in parameters_to_fit:
            row = df[df["parameter_names"] == param].iloc[0]
            assert row["sigma_minus"] > 0

    def test_format_fitted_sigma_plus_are_positive(
        self, df, parameters_to_fit
    ):
        for param in parameters_to_fit:
            row = df[df["parameter_names"] == param].iloc[0]
    ...
    def test_format_fitted_params_before_fixed_params(
        self, df, parameters_to_fit
    ):
...
class TestMMEXOFASTFitResultsWithEmceeFixedParams:
    ...
    def test_format_fixed_s_sigma_minus_is_nan(self, df):
        assert np.isnan(
            df[df["parameter_names"] == "s"].iloc[0]["sigma_minus"]
        )
    ...
    def test_format_fixed_q_sigma_minus_is_nan(self, df):
        assert np.isnan(
            df[df["parameter_names"] == "q"].iloc[0]["sigma_minus"]
        )
