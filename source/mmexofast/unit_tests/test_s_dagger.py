...
KB171003 = {
    ...
    "expected": {
        ...
        "s_mean": 0.899,
        "log_q": -3.6,
    },  # actual (rather than heuristic) mean
}
...
class SDaggerTestBase:
    ...
    def _get_anom_params(self, solution):
        anom_params = self.event[
            "anomaly_lc_params"
        ].copy()  # avoid mutating shared fixture
    ...
    def _assert_expected_matches_exact_result(self, result):
        for key, value in self.event["expected"].items():
            if key == "alpha":
                ...
                assert abs(1.0 - result["s"] / value) < self.s_dagger_tol
            elif key == "log_q":
                assert (
                    abs(np.log10(result["q"]) - value) < 0.5
                )  # Unlikely to be terribly accurate.
    ...
    def _check_s_dagger(self, solution):
        ...
        ratio = (
            self.event["expected"]["s_mean"] ** 2
            / self.event[solution]["s"]
            / alt_sol["s"]
        )
        assert abs(1.0 - ratio) < self.s_dagger_tol, (
