...
def _get_co_mass_co_manif_offset(params):
    ...
    """
    if params["s"] > 1.0:
        delta_x = (params["q"] / (1.0 + params["q"])) * (
            (1.0 / params["s"]) - params["s"]
        )
        delta_u0 = delta_x * np.sin(np.deg2rad(params["alpha"]))
        delta_t0 = (
            delta_x * params["t_E"] * np.cos(np.deg2rad(params["alpha"]))
        )
    else:
        delta_t0 = 0.0
        delta_u0 = 0.0
...
def co_mass_to_co_magnif(params):
    ...
    return {"t_0": params["t_0"] - delta_t0, "u_0": params["u_0"] - delta_u0}
...
def primary_to_co_magnif(params):
    ...
    delta_x = params["q"] * ((1.0 / params["s"]) + params["s"])
    delta_u0 = delta_x * np.sin(np.deg2rad(params["alpha"]))
    delta_t0 = delta_x * params["t_E"] * np.cos(np.deg2rad(-params["alpha"]))
