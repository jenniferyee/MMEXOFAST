...
event_info = np.genfromtxt(
    os.path.join(dir_, "event_info.txt"),
    dtype=None,
    encoding="utf-8",
...
class TestDataSet:
    def __init__(self, lc_num):
        ...
        coords = SkyCoord(
            event_info["ra"][index][0], event_info["dec"][index][0], unit="deg"
        )
...
class DC18Answers:
    ...
    def __init__(self):
        columns = np.genfromtxt(
            os.path.join(dir_, "Answers", "wfirstColumnNumbers.txt"),
            dtype=None,
            encoding="utf-8",
            usecols=[0, 1],
            skip_header=2,
...
if __name__ == "__main__":
    ...
    index = (
        (answers.data["s"] < 0.5)
        & (answers.data["u0"].abs() > 0.1)
        & (answers.data["q"] > 0.001)
    )
    with pd.option_context("display.width", None, "display.max_rows", None):
        print(
            answers.data[answers.print_columns][index]
        )  # .iloc[np.array([139, 193, 217, 250, 289])-1])
