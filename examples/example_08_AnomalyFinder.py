...
class AnomalyFinderTest:
    ...
    def _compute_t_range(self):
        ...
        times = data.time[np.abs(data.time - t_0) < 180.0]
    ...
    def plot_chi2_grid(self):
        ...
        for j in range(4):
            ...
            plt.figure()
            plt.scatter(
                self.af.grid_t_0[sorted_idx],
                self.af.grid_t_eff[sorted_idx],
    ...
    def plot_event(self):
        ...
        event = MulensModel.Event(
            datasets=self.datasets, model=MulensModel.Model(self.pspl_params)
        )
        ...
        print(ref_fluxes)
        model_fluxes = (
            ref_fluxes[0][0] * event.model.get_magnification(times)
            + ref_fluxes[1]
        )
...
if __name__ == "__main__":
    lc_num = 4
    pspl_params = {
        "t_0": 2460023.2869408596,
        "t_E": 5.633313663946914,
        "u_0": 2.216056444359401,
    }
