...
data = mm.MulensData(
    file_name=os.path.join(MULENS_DATA_PATH, "OB140939", "ob140939_OGLE.dat"),
    phot_fmt="mag",
)
# Initial model
model = mm.Model({"t_0": 2456836.2, "u_0": 0.9, "t_E": 23.0})
...
with pm.Model() as no_grad_model:
    ...
    t_0 = pm.Uniform(
        "t_0",
        lower=model.parameters.t_0 - 5.0,
        upper=model.parameters.t_0 + 5.0,
        initval=model.parameters.t_0,
    )
    u_0 = pm.Uniform("u_0", lower=0.5, upper=1.5, initval=model.parameters.u_0)
    t_E = pm.Uniform(
        "t_E", lower=10.0, upper=40.0, initval=model.parameters.t_E
    )
