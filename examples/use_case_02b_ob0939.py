...
class ParallaxFitter:
    ...
    def plot_lc(self, savefig=None, **kwargs):
        ...
        model = self.fitter.all_fit_results.get(
            "PSPL static "
        ).full_result.fitter.get_model()
        model.parameters = self.fitter.all_fit_results.get(
            "PSPL static "
        ).params
    ...
    def plot_parallax_grids(self, n_scale=1, savefig=None, **kwargs):
        ...
        for idx, u0sign in enumerate(["+", "-"]):
            ...
            best_chi2 = self.fitter.results[
                "PSPL par ({0}u_0)".format(u0sign)
            ].best["chi2"]
            plt.sca(axes[idx])
            plt.scatter(
                grid["pi_E_E"],
                grid["pi_E_N"],
                c=np.sqrt(grid["chi2"] - best_chi2),
                vmin=0,
                vmax=n_scale * 9,
                cmap="Set1",
            )
...
class SpaceParallaxFitter(ParallaxFitter):
    def __init__(
        self, ground_data_files=None, space_data_files=None, **kwargs
    ):
...
def do_ground_fit():
    ...
    pl_fitter.save_results_table(
        filename="test_output/OB0939_gr_fits.tex", type="latex"
    )
...
def do_complete_fit():
    par_fitter = SpaceParallaxFitter(
        ground_data_files=ground_data_files,
        space_data_files=space_data_files,
        coords=coords,
        fit_type="point lens",
    )
    par_fitter.do_fits()
    par_fitter.save_results_table(
        filename="test_output/OB0939_fits.tex", type="latex"
    )

    par_fitter.plot_space_and_ground_lc(
        savefig="test_output/OB0939_lc.eps", dpi=300
    )
    par_fitter.plot_parallax_contours(
        savefig="test_output/OB0939_par_contours.eps", dpi=300
    )
